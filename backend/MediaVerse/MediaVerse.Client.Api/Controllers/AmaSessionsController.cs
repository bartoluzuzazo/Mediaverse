...
public class AmaSessionsController(IMediator mediator) : BaseController
{
    ...
    public async Task<IActionResult> PostQuestion(Guid sessionId,PostAmaQuestionDto postAmaQuestionDto)
    {
        ...
        var response = await mediator.Send(command);
        //TODO: remove that lie
        return ResolveCode(response.Exception, CreatedAtAction(nameof(GetAmaSession), new {sessionId = sessionId}, response.Data));
    }
    ...
    [Authorize]
    public async Task<IActionResult> GetAuthorizedQuestions(Guid sessionId, int page, int size, QuestionOrder order, OrderDirection direction )
    {
