...
const AuthorEntryPreview : FunctionComponent<Props> = ({entry}) => {
  ...
  return (
    <div className="flex flex-row justify-between cursor-pointer" onClick={handleLink}>
      <img
        src={imgSrc}
        className="aspect-square w-36 h-36 border-4 border-white bg-slate-300 p-2"
        alt="cover photo"
      />
      <div className="flex flex-col p-2 font-bold">
        ...
        <div className="flex flex-row">
          ...
          <h2 className="p-2">{entry.avgRating}★</h2>
        </div>
      </div>
      <p className="p-2 w-8/12">
        {entry.description.length > maxDescLen ? entry.description.substring(0, maxDescLen)+" [...]" : entry.description}
      </p>
    </div>
  )
}
